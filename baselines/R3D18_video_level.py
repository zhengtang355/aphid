...
class AphidVideoFolderDataset(Dataset):
    def __init__(self, root, num_clips=16, frames_per_clip=16, frame_stride=1, image_size=112, train=True):
        self.root = Path(root)
...
def build_r3d18(num_classes=2, pretrained=True):
...
def main():
    ...
    parser.add_argument("--save_path", type=str, default="best_r3d18_video_level.pth")
    parser.add_argument("--log_path", type=str, default="r3d18_train_log.csv")
    ...
    model = build_r3d18(num_classes=2, pretrained=args.pretrained).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"val_ap={metrics['ap']:.4f} val_acc={metrics['acc']:.4f} val_p={metrics['p']:.4f} "
            f"val_r={metrics['r']:.4f} val_f1={metrics['f1']:.4f}",
            flush=True,
        )
